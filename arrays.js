...
function isPresent(studentX) {
  ...
  if (bool) console.log(`${studentX} is present.`);
  if (!bool) console.log(`${studentX} is absent.`);
}
isPresent("Ali");
...
You are creating a leaderboard for a soccer game. The array `topScorers` contains
the names of players and their scores.
...
2. Write another function called `printLeaderboard` that sorts the leaderboard in descending order of
scores and prints it.
...
function updateScore(plrName, scoreNum) {
  let bool = false;
  for (const plr of leaderboard) {
    if (plr.name === plrName) {
      plr.score = scoreNum;
      bool = true;
    }
  ...
  if (!bool) {
    leaderboard.push({ name: plrName, score: scoreNum });
  }
...
const clues = ["Map", "Compass", "Key", "Shovel"];
const clueMessages = ["paM", "ssapmoC", "yeK", "levohS"]; // scrambled clues
...
function findClue(clueArr, clueName) {
  ...
  } else console.log(`Clue ${clueName} is missing, search again!`);
}
findClue(clues, "Shovel")

/* 
function decipherMessage(clueArr) {
  let arrLength = clueArr.length;
  for (const clue of clueArr) {
    const reversedV1 = clue.split("");
    const reversedV2 = reversedV1.reverse();
    const reversedV3 = reversedV2.join("");
    clueArr.push(reversedV3);
  }
  clueArr.slice(arrLength);
  console.log(clueArr);
}
decipherMessage(clueMessages);
 */